...
export function PeriodicRow({ rowNumber, elementsByPosition, onElementClick }: PeriodicRowProps) {
  const getElement = (row: number, column: number) => elementsByPosition[`${row}-${column}`];

  const renderCell = (column: number) => {
    const element = getElement(rowNumber, column);
    return element ? (
      <ElementCell key={element.atomicNumber} element={element} onClick={onElementClick} />
    ) : (
  ...
  const Cells = () => {
    if (rowNumber === 1) {
      return [
        renderCell(1),
        ...Array.from({ length: 16 }, (_, i) => <EmptyCell key={`${rowNumber}-${i + 2}`} />),
        renderCell(18),
      ];
    }
    ...
    if (rowNumber === 2 || rowNumber === 3) {
      return [
        renderCell(1),
        renderCell(2),
        ...Array.from({ length: 10 }, (_, i) => <EmptyCell key={`${rowNumber}-${i + 3}`} />),
...
export function SpecialRow({ category, elements, onElementClick }: SpecialRowProps) {
